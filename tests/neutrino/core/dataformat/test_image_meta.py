...
def test_filled_constructor(dimension, execution_number):
    ...
    number_of_voxels = neutrino.VectorOfSizet()
    image_sizes = neutrino.VectorOfDouble()
    origin = neutrino.VectorOfDouble()
    ...
    if dimension == 2:
        im = neutrino.ImageMeta2D(projection_id, number_of_voxels, image_sizes)
    elif dimension == 3:
        im = neutrino.ImageMeta3D(projection_id, number_of_voxels, image_sizes)
    else:
        assert(False)
...
def test_ravel(dimension, execution_number):
    ...
    for i in range(50):
        ...
        indexes = []
        vec_of_indexes = neutrino.VectorOfSizet()
        for dim in dims:
