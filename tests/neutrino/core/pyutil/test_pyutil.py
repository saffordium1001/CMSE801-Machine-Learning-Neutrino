...
#     vec = neutrino.VectorOfFloat();
#     for i in range(50):
#         vec.append(i)
#     print(vec)
#     print(type(vec))
#     ndarray = neutrino.as_ndarray_float(vec)
...
def test_image2d_conversion(x_dim, y_dim):
    ...
    image2d = neutrino.as_image2d(numpy_image)

    # Convert the image back to numpy array:

    numpy_image_back = neutrino.as_ndarray(image2d)
...
def test_image2d_conversion_with_meta(x_dim, y_dim):
    ...
    # Convert the numpy array to image 2D:
    image2d = neutrino.as_image2d_meta(numpy_image, meta)
