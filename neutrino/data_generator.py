...
def write_image2d(file_name, event_image_list):
    ...
    io_manager = neutrino.IOManager(neutrino.IOManager.kWRITE)
    ...
    for event in range(len(event_image_list)):
        ...
        ev_image2d = neutrino.EventImage2D.to_image2d(io_manager.get_data("image2d","test"))

        for projection in range(len(images)):

            image2d = neutrino.as_image2d(images[projection])
...
def read_image2d(file_name):
    ...
    io_manager = neutrino.IOManager(neutrino.IOManager.kREAD)
    ...
    for i in range(io_manager.get_n_entries()):
        ...
        for projection in range(ev_image2d.as_vector().size()):
            image = neutrino.as_ndarray(ev_image2d.as_vector()[projection])
...
def write_tensor(file_name, event_image_list, dimension):
    ...
    for event in range(len(event_image_list)):
        ...
        if dimension == 1:
            ev_tensor = neutrino.EventTensor1D.to_tensor(io_manager.get_data("tensor1d","test"))
        if dimension == 2:
            ev_tensor = neutrino.EventImage2D.to_tensor(io_manager.get_data("image2d","test"))
        if dimension == 3:
            ev_tensor = neutrino.EventTensor3D.to_tensor(io_manager.get_data("tensor3d","test"))
        if dimension == 4:
            ev_tensor = neutrino.EventTensor4D.to_tensor(io_manager.get_data("tensor4d","test"))

        for projection in range(len(images)):

            if dimension == 1: tensor = neutrino.as_tensor1d(images[projection])
            if dimension == 2: tensor = neutrino.as_image2d(images[projection])
            if dimension == 3: tensor = neutrino.as_tensor3d(images[projection])
            if dimension == 4: tensor = neutrino.as_tensor4d(images[projection])
...
def read_tensor(file_name, dimensions):
    ...
    for i in range(io_manager.get_n_entries()):
        ...
        for projection in range(ev_tensor.as_vector().size()):
            image = neutrino.as_ndarray(ev_tensor.as_vector()[projection])
            event_image_list[i].append(copy(image))
...
def write_sparse_clusters(file_name, voxel_set_array_list, dimension=2, n_projections=3):
    ...
    for projection in range(n_projections):
        if dimension == 2:
            meta_list.append(neutrino.ImageMeta2D())
        else:
            meta_list.append(neutrino.ImageMeta3D())
    ...
    for i in range(len(voxel_set_array_list)):
        ...
        # Get a piece of data, sparse tensor:
        if dimension== 2:
            ev_cluster = neutrino.EventSparseCluster2D.to_sparse_cluster(io_manager.get_data("cluster2d","test"))
        else:
            ev_cluster = neutrino.EventSparseCluster3D.to_sparse_cluster(io_manager.get_data("cluster3d","test"))
        ...
        for projection in range(n_projections):
            ...
            if dimension == 2:
                vsa = neutrino.SparseCluster2D()
            else:
                vsa = neutrino.SparseCluster3D()
...
def write_sparse_tensors(file_name, voxel_set_list, dimension, n_projections):
    ...
    for i in range(len(voxel_set_list)):
        ...
        # Get a piece of data, sparse tensor:
        if dimension== 2:
            ev_sparse = neutrino.EventSparseTensor2D.to_sparse_tensor(io_manager.get_data("sparse2d","test"))
        else:
            ev_sparse = neutrino.EventSparseTensor3D.to_sparse_tensor(io_manager.get_data("sparse3d","test"))
...
def write_particles(tempfile, rand_num_events, particles_per_event=-1):
    ...
    for i in range(rand_num_events):
        ...
        ev_particle = neutrino.EventParticle.to_particle(io_manager.get_data("particle","test"))
...
def read_particles(tempfile, use_core_driver=False):
    ...
    for i in range(io_manager.get_n_entries()):
        ...
        ev_particles = neutrino.EventParticle.to_particle(io_manager.get_data('particle', 'test'))
        read_events += 1
