...
class batch_pydata(object): #stores the incoming information from the convolutional neural network as it is applied to the images
   ...
   def set_data(self,storage_id,neutrino_batchdata):
      self._storage_id = storage_id
      dim = neutrino_batchdata.dim()
      ...
      if self._make_copy:
         if self._npy_data is None:
            # self._npy_data = np.array(neutrino_batchdata.data())
            # print("Array type: ", self._npy_data.dtype)
            # Create an array to hold the data if it does not exits:
            self._npy_data = np.ndarray(shape=(neutrino_batchdata.data_size()), dtype=self._dtype)
         self._npy_data = self._npy_data.reshape(self.batch_data_size())
         self._npy_data = np.copy(neutrino_batchdata.pydata())

      else:
         self._npy_data = neutrino_batchdata.pydata()
