...
class neutrino_writer(object):

    '''A utility class for writing data as neutrino objects to file.
     Meant to package everything into an easy to use interface for writing output files.(Saves mine
     and hopefully your sanity as well)
    '''

    def __init__(self, io_config, output_file=None):
        '''neutrino_writer accesses an IOManager instance directly to manage writing
        ...
        self._io = neutrino.IOManager(io_config)
    ...
    def _write_sparse2d(self, data, producer):
        ...
        Data is writting under the provided producer name. contains a dict with the following
        three elements:
        ...
        '''
        _writable_data = neutrino.EventSparseTensor2D.to_sparse_tensor(self._io.get_data("sparse2d", producer))
        ...
        for projection in range(len(data)):
            ...
            voxel_set = neutrino.as_voxelset(value.astype(numpy.float32), index.astype(numpy.uint64))
            # Add the voxel set:
    ...
    def _write_image2d(self, data, producer):
        ...
        ev_image2d = neutrino.EventImage2D.to_image2d(self._io.get_data("image2d", producer))
        ...
        for projection_id, image in enumerate(data):
            ...
            img = neutrino.as_image2d_meta(image, meta)
    ...
    def _write_tensor1d(self, data, producer):
        ...
        ev_tensor = neutrino.EventTensor1D.to_tensor(self._io.get_data("tensor1d", producer))

        tensor = neutrino.as_tensor1d(data)
    ...
    def write(self, data, datatype, producer, entries, event_ids):
        ...
        Arguments:
            ...
            datatype {str} -- neutrino2 datatype descriptor
            producer {srt} -- producer key underwhich to store this data
            entries {vector<int>} -- vector of file entries, size 1 for now
            event_ids {vector<neutrino::EventBase>} -- vector of neutrino EventBase objects, size 1 for now
        '''
