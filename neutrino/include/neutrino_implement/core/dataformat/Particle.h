...
#ifndef __NEUTRINO_IMPLEMENT_DATAFORMAT_PARTICLE_H__
#define __NEUTRINO_IMPLEMENT_DATAFORMAT_PARTICLE_H__

#include <iostream>
#include "neutrino_implement/core/dataformat/Vertex.h"
// #include "BBox.h"
...
namespace neutrino_implement {
  ...
  */
  ...
  public:
    ...
    /// Default constructor
    Particle(neutrino_implement::ShapeType_t shape=neutrino_implement::kShapeUnknown)
      : _id         (kINVALID_INDEX)
...
// _ancestor_vtx
    ...
    inline const neutrino_implement::Vertex& position() const { return _vtx;   }
    ...
    inline const neutrino_implement::Vertex& end_position () const { return _end_pt;     }
    inline const neutrino_implement::Vertex& first_step   () const { return _first_step; }
    inline const neutrino_implement::Vertex& last_step    () const { return _last_step;  }
    ...
    // inline const std::vector<neutrino_implement::BBox2D>& boundingbox_2d() const { return _bb2d_v; }
    ...
    inline const neutrino_implement::Vertex& parent_position () const { return _parent_vtx;}
    ...
    inline const neutrino_implement::Vertex& ancestor_position () const { return _ancestor_vtx;}
    ...
    inline void position        (const neutrino_implement::Vertex& vtx) { _vtx = vtx;    }
    inline void position        (double x, double y, double z, double t) { _vtx = Vertex(x,y,z,t); }
    inline void end_position    (const neutrino_implement::Vertex& vtx) { _end_pt = vtx; }
    inline void end_position    (double x, double y, double z, double t) { _end_pt = Vertex(x,y,z,t); }
    inline void first_step      (const neutrino_implement::Vertex& vtx) { _first_step = vtx; }
    inline void first_step      (double x, double y, double z, double t) { _first_step = Vertex(x,y,z,t); }
    inline void last_step       (const neutrino_implement::Vertex& vtx) { _last_step = vtx; }
    ...
    // inline void boundingbox_2d(const std::vector<neutrino_implement::BBox2D>& bb_v) { _bb2d_v = bb_v; }
    ...
    inline void parent_position (const neutrino_implement::Vertex& vtx) { _parent_vtx = vtx; }
    ...
    inline void ancestor_position (const neutrino_implement::Vertex& vtx) { _ancestor_vtx = vtx; }
  ...
  public:
    static hid_t get_datatype() {
      ...
      status = H5Tinsert(datatype, "id",               HOFFSET(Particle, _id),               neutrino_implement::get_datatype<InstanceID_t>());
      status = H5Tinsert(datatype, "mcst_index",       HOFFSET(Particle, _mcst_index),       neutrino_implement::get_datatype<MCSTIndex_t>());
      status = H5Tinsert(datatype, "mct_index",        HOFFSET(Particle, _mct_index),        neutrino_implement::get_datatype<MCTIndex_t>());
      status = H5Tinsert(datatype, "shape",            HOFFSET(Particle, _shape),            neutrino_implement::get_datatype<ShapeType_t>());
      status = H5Tinsert(datatype, "current_type",     HOFFSET(Particle, _current_type),     neutrino_implement::get_datatype<short>());
      status = H5Tinsert(datatype, "interaction_type", HOFFSET(Particle, _interaction_type), neutrino_implement::get_datatype<short>());
      status = H5Tinsert(datatype, "trackid",          HOFFSET(Particle, _trackid),          neutrino_implement::get_datatype<unsigned int>());
      status = H5Tinsert(datatype, "pdg",              HOFFSET(Particle, _pdg),              neutrino_implement::get_datatype<int>());
      status = H5Tinsert(datatype, "px",               HOFFSET(Particle, _px),               neutrino_implement::get_datatype<double>());
      status = H5Tinsert(datatype, "py",               HOFFSET(Particle, _py),               neutrino_implement::get_datatype<double>());
      status = H5Tinsert(datatype, "pz",               HOFFSET(Particle, _pz),               neutrino_implement::get_datatype<double>());
      ...
      status = H5Tinsert(datatype, "dist_travel",      HOFFSET(Particle, _dist_travel),      neutrino_implement::get_datatype<double>());
      status = H5Tinsert(datatype, "energy_init",      HOFFSET(Particle, _energy_init),      neutrino_implement::get_datatype<double>());
      status = H5Tinsert(datatype, "energy_deposit",   HOFFSET(Particle, _energy_deposit),   neutrino_implement::get_datatype<double>());
      status = H5Tinsert(datatype, "process",          HOFFSET(Particle, _process),          string_type);
      status = H5Tinsert(datatype, "parent_trackid",   HOFFSET(Particle, _parent_trackid),   neutrino_implement::get_datatype<unsigned int>());
      status = H5Tinsert(datatype, "parent_pdg",       HOFFSET(Particle, _parent_pdg),       neutrino_implement::get_datatype<int>());
      status = H5Tinsert(datatype, "parent_vtx",       HOFFSET(Particle, _parent_vtx),       Vertex::get_datatype());
      status = H5Tinsert(datatype, "ancestor_trackid", HOFFSET(Particle, _ancestor_trackid), neutrino_implement::get_datatype<unsigned int>());
      status = H5Tinsert(datatype, "ancestor_pdg",     HOFFSET(Particle, _ancestor_pdg),     neutrino_implement::get_datatype<int>());
  ...
  private:
    ...
    // std::vector<neutrino_implement::BBox2D> _bb2d_v; ///< bounding box of particle's trajectory in 2D projections. index = ProjectionID_t
    // neutrino_implement::BBox3D _bb3d; ///< bounding box of particle's trajectory in 3D
