#ifndef __NEUTRINO_IMPLEMENT_DATAFORMAT_VERTEX_H__
#define __NEUTRINO_IMPLEMENT_DATAFORMAT_VERTEX_H__

#include "neutrino_implement/core/dataformat/DataFormatTypes.h"
#include "neutrino_implement/core/dataformat/Point.h"
...
namespace neutrino_implement {
  ...
  public:
    ...
    /// Convert to point
    const neutrino_implement::Point2D as_point2d(neutrino_implement::PointType_t point_type) const;
    const neutrino_implement::Point3D as_point3d() const;
    void as_point(neutrino_implement::PointType_t point_type, double * x, double * y, double * z);
  ...
  public: 
    static hid_t get_datatype() {
      ...
      status = H5Tinsert (datatype, "x",
                  HOFFSET (Vertex, _x), neutrino_implement::get_datatype<double>());
      status = H5Tinsert (datatype, "y",
                  HOFFSET (Vertex, _y), neutrino_implement::get_datatype<double>());
      status = H5Tinsert (datatype, "z",
                  HOFFSET (Vertex, _z), neutrino_implement::get_datatype<double>());
      status = H5Tinsert (datatype, "t", 
                  HOFFSET (Vertex, _t), neutrino_implement::get_datatype<double>());
      return datatype;
    }
#endif
