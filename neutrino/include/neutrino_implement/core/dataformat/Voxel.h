...
#ifndef __NEUTRINO_IMPLEMENT_DATAFORMAT_VOXEL_H
#define __NEUTRINO_IMPLEMENT_DATAFORMAT_VOXEL_H
...
namespace neutrino_implement {
  ...
  */
  ...
  public:
    static hid_t get_datatype() {
      ...
      status = H5Tinsert (datatype, "id",
                  HOFFSET (Voxel, _id), neutrino_implement::get_datatype<unsigned long>());
      status = H5Tinsert (datatype, "value",
                  HOFFSET (Voxel, _value), neutrino_implement::get_datatype<float>());
      return datatype;
    }
#endif
  ...
  // static const neutrino_implement::Voxel kINVALID_VOXEL(kINVALID_VOXELID,0.);
  ...
  public:
    ...
    inline const std::vector<neutrino_implement::Voxel>& as_vector() const { return _voxel_v; }
  ...
  private:
    ...
    std::vector<neutrino_implement::Voxel> _voxel_v;
  };
  ...
  */
  ...
  public:
    ...
    const neutrino_implement::VoxelSet& voxel_set(InstanceID_t id) const;
    /// Access all VoxelSet as a vector
    inline const std::vector<neutrino_implement::VoxelSet>& as_vector() const
    { return _voxel_vv; }
    float sum() const;
    /// Mean of contained voxel values
    float mean() const;
    ...
    neutrino_implement::VoxelSet& writeable_voxel_set(const InstanceID_t id);
    /// Move an arrray of VoxelSet. Each element's InstanceID_t gets updated
    void emplace(std::vector<neutrino_implement::VoxelSet>&& voxel_vv);
    /// Set an array of VoxelSet. Each element's InstanceID_t gets updated
    // void insert(const std::vector<neutrino_implement::VoxelSet>& voxel_vv);
    /// Move a VoxelSet into a collection. The InstanceID_t is respected.
    void emplace(neutrino_implement::VoxelSet&& voxel_v);
    /// Set a VoxelSet into a collection. The InstanceID_t is respected.
    void insert(const neutrino_implement::VoxelSet& voxel_v);
    /// Mover
    void move(neutrino_implement::VoxelSetArray&& orig)
    { _voxel_vv = std::move(orig._voxel_vv); }

  private:
    std::vector<neutrino_implement::VoxelSet> _voxel_vv;
  };
  ...
  */
  ...
  public:
    ...
    inline const neutrino_implement::ImageMeta<dimension>& meta() const { return _meta; }

    //
    // Write-access
    //
    /// Create & add/set a single voxel
    void emplace(const neutrino_implement::Voxel & vox, const bool add=true);
    ...
    /// Meta setter
    void meta(const neutrino_implement::ImageMeta<dimension>& meta);

  private:
    neutrino_implement::ImageMeta<dimension> _meta;
