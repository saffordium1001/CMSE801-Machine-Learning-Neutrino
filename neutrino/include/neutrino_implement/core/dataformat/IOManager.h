...
#ifndef __NEUTRINO_IMPLEMENT_DATAFORMAT_IOMANAGER_H
#define __NEUTRINO_IMPLEMENT_DATAFORMAT_IOMANAGER_H
...
#include "neutrino_implement/core/dataformat/EventID.h"
...
namespace neutrino_implement {
  ...
  */
  class IOManager : public neutrino_implement::neutrino_base {
  ...
  public:
    ...
    inline T& get_data(const ProducerID_t id)
    {
      if (id >= _product_type_v.size()) {
        NEUTRINO_CRITICAL() << "Invalid producer id: " << id << " requested " << std::endl;
        throw neut();
      }
      auto ptr = this->get_data(id);
      if (product_unique_name<T>() != _product_type_v[id]) {
        NEUTRINO_CRITICAL() << "Unmatched type (in memory type = " << _product_type_v[id]
  ...
  protected:
    ...
    std::map<neutrino_implement::ProducerName_t, neutrino_implement::ProducerID_t> _key_list;
    // This is the key list for all producers in the input file only:
    std::map<neutrino_implement::ProducerName_t, neutrino_implement::ProducerID_t> _in_key_list;
    ...
    size_t                          _product_ctr;
    std::vector<neutrino_implement::EventBase*> _product_ptr_v;
