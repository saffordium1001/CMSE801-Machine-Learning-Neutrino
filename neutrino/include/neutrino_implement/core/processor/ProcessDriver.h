...
#ifndef __NEUTRINO_IMPLEMENT_PROCESSOR_PROCESSDRIVER_H
#define __NEUTRINO_IMPLEMENT_PROCESSOR_PROCESSDRIVER_H
...
namespace neutrino_implement {
  /**
     \class ProcessDriver
     @brief Analysis event-loop driver that executes neutrino_implement::ProcessBase inherit "process modules".
     It uses neutrino_implement::IOManager behind the scene to run the event loop.\n
     Responsible for propagating a structured configuration parameter sets to configure each process module.\n
     The configuration of neutrino_implement::ProcessDriver itself can contain neutrino_implement::IOManager configuration.
  */
  class ProcessDriver : public neutrino_base {
  ...
  public:
    ...
    //
    /// Decodes given configuration text file to create neutrino_implement::PSet, then calls the other configure function.
    void configure(const std::string config_file);
    /// Configure itself (mainly neutrino_implement::IOManager) and also attached process modules.
    ...
    void reset();
    /// Must be called after configure. This initializes neutrino_implement::IOManager (open files) and initialize attached process modules.
    ...
    */
    ...
    //
    /// Returns neutrino_implement::EventBase object that contains an "ID" (run/event integers)
    inline const EventID& event_id() const
    { return ( _io.io_mode() == neutrino_implement::IOManager::kREAD ? _io.event_id() : _io.last_event_id()); }
    ...
    /// Returns read-only neutrino_implement::IOManager instance
  ...
  protected:
    ...
    IOManager _io;
    std::map<std::string,neutrino_implement::ProcessID_t> _proc_m;
    std::vector<neutrino_implement::ProcessBase*> _proc_v;
    bool _processing;
