#ifndef __NEUTRINO_IMPLEMENT_CPPUTIL_CSVREADER_H__
#define __NEUTRINO_IMPLEMENT_CPPUTIL_CSVREADER_H__
...
namespace neutrino_implement {
  ...
  public:
    ...
    /// alternative ctor
    CSVData(const std::string& fname, const std::vector<neutrino_implement::CSVData::ValueType_t>& format)
    { read_file(fname,format); }
    ...
    /// Read in CSV file
    void read_file(const std::string, const std::vector<neutrino_implement::CSVData::ValueType_t>&);
  ...
  private:
    void read_line(const std::string& line_data,
		   const std::vector<neutrino_implement::CSVData::ValueType_t>& value_types,
  ...
  inline const std::vector<T>& CSVData::get(const std::string key) const
  {
    ...
    if(iter == data_m.end()) {
      NEUTRINO_SCRITICAL() << "Key " << key << " does not exist!" << std::endl;
      throw neut();
    }
    return (*iter).second;
  }
