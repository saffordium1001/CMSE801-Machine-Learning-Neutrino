...
#ifndef __NEUTRINO_IMPLEMENT_BASE_PSET_H__
#define __NEUTRINO_IMPLEMENT_BASE_PSET_H__

#include <iostream>
#include <string>
#include <map>
#include "neutrino_implement/core/base/neut.h"
#include "neutrino_implement/core/base/Parser.h"
#include "neutrino_implement/core/base/neutrino_logger.h"

namespace neutrino_implement {
  /**
     \class PSet
     \brief A nested configuration parameter set holder for neutrino_implement framework.
  */
  class PSet {
  ...
  public:
    ...
    T get(const std::string& key) const{
      ...
      if( iter == _data_value.end() ) {
        ...
        NEUTRINO_CRITICAL() << msg << std::endl;
        throw neut(msg);
      }
  ...
  private:
    ...
    /// The name of this neutrino_implement::PSet
    std::string _name;
    /// Key-Value pairs
    std::map<std::string,std::string> _data_value;
    /// Key-PSet pairs
    std::map<std::string,::neutrino_implement::PSet> _data_pset;

  };

  template<> PSet PSet::get<neutrino_implement::PSet>(const std::string& key) const;
