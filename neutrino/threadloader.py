#import necessary Python modules for the system, time, OS, numpy for Math, etc.
#from . import neutrino #gives errors in regards to relative directories - didn't have time to change but can fix later
import sys,time,os,signal
import numpy as np
#import the neutrino classes and module
from batch_pydata import batch_pydata
from neutrino_writer import neutrino_writer


class thread_interface(object):
    '''
    This class is just a wrapper around the dataloaders and threadio. This was intended to be used for parallel computing
    should I under time constraints have had the opportunity to use Tensorflow. 
    This class implements the same interface as the distributed_neutrino_interface class
    which wraps neutrino threadio in mpi4py to allow distributed training. See that class for
    more details.
    '''
    ...
    def prepare_writer(self, io_config, output_file=None): #prepares for writing to script
        ...
        # The writer is not an instance of threadIO but rather an instance of neutrino_writer.

        # It configures a process to copy input to output and add more information as well.

        self._writer = neutrino_writer(io_config, output_file)
    ...
    def prepare_manager(self, mode, io_config, minibatch_size, data_keys):
        ...
        Creates an instance of neutrino_threadio for a particular file to read.
...
class neutrino_threadio (object): #class inheritance for input-output thread
   ...
   def configure(self,cfg,color=0):
      ...
      self._proc = neutrino.ThreadProcessor(self._name)

      # We have to pass a color here.  Otherwise, if we use this and neutrino is built with mpi, it will hang
      ...
      for i in range(len(self._proc.batch_fillers())):
         ...
         dtype = neutrino.BatchDataTypeName(self._proc.batch_types()[i])
   ...
   def next(self,store_entries=False,store_event_ids=False):
      ...
      for name,storage in self._storage.items():
         dtype = storage.dtype()
         if dtype == "float32":
            factory = neutrino.BatchDataStorageFactoryFloat.get()
         elif dtype == "float64":
            factory = neutrino.BatchDataStorageFactoryDouble.get()
         elif dtype == "int":
            factory = neutrino.BatchDataStorageFactoryInt.get()
      ...
      else: self._tree_entries = neutrino.VectorOfSizet(self._proc.processed_entries(next_storage_id))

      if not store_event_ids: self._event_ids = None
      else: self._event_ids = neutrino.VectorOfEventID(self._proc.processed_events(next_storage_id))
...
def sig_kill(signal,frame):
   ...
   for name,ptr in neutrino_threadio._instance_m.items():
