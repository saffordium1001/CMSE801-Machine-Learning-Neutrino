...
from neutrino_io_enums import RandomAccess
...
class queue_interface(object):
    ...
    def __init__(self, verbose=False, random_access_mode="random_blocks"):
        ...
        A dict. of dataloaders and the keys to access their data.
    ...
    def prepare_writer(self, io_config, output_file=None):
        ...
        # The writer is not an instance of queueIO but rather an instance of neutrino_writer.
    ...
    #function here is self-explanatory - stops writing
    def stop(self):
    ...
    def ready(self, mode):
        #determines whether or not the queueloaders are ready
...
class neutrino_queueio (object):
    ...
    def configure(self,cfg, color=0):
        ...
        self._proc = neutrino.QueueProcessor(self._name)
    ...
    def set_next_batch(self, batch_indexes):
        if type(batch_indexes) != neutrino.VectorOfSizet:
            indexes = neutrino.VectorOfSizet()
    ...
    def next(self,store_entries=False,store_event_ids=False):
        ...
        for name,storage in self._storage.items():
            dtype = storage.dtype()
            if dtype == "float32":
                factory = neutrino.BatchDataQueueFactoryFloat.get()
            elif dtype == "float64":
                factory = neutrino.BatchDataQueueFactoryDouble.get()
            elif dtype == "int":
                factory = neutrino.BatchDataQueueFactoryInt.get()
            ...
            storage.set_data(storage_id=name, neutrino_batchdata=batch_data)
    ...
    #all functions below this line are to fetch information about data, data entries, identifications, etc.
