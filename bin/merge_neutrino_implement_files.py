...
def main():
  ...
  file_list = args.neutrino_fin
  if type(file_list) == str:
    file_list = [file_list,]

  output_file = h5py.File(args.neutrino_fout, 'w')
...
def copy_event_id_table(input_file, output_file):
  ...
  if 'event_id' not in output_file["Events/"]:
    ...
    # The dataset already exists, so let's resize and assign it new elements:
...
def copy_data_product(input_file, output_file, group_name):
  ...
  # Converts a data product. This will mean copying all tables from a group into
  ...
  for dataset in input_file[full_path].keys():
    ...
    if dataset not in output_file[full_path]:
      ...
      # print ("Appending to dataset ", dataset) //just here for placeholder until determined otherwise what I want to do here
